...
func NewResourceMap() *ResourceMap {
	return &ResourceMap{
		m: make(map[ResourceID]*ResourceStatus),
	}
}
...
func (rm *ResourceMap) UnsafeGet() map[ResourceID]*ResourceStatus {
	return rm.m
}
...
func (rm *ResourceMap) FindWithDefault(k ResourceID, dV *ResourceStatus) *ResourceStatus {
	rm.rwMu.RLock()
...
func (rm *ResourceMap) InsertOrUpdate(k ResourceID, val *ResourceStatus) bool {
	rm.rwMu.Lock()
...
func (rm *ResourceMap) InsertIfNotPresent(k ResourceID, val *ResourceStatus) bool {
	rm.rwMu.Lock()
	defer rm.rwMu.Unlock()
	_, ok := rm.m[k]
	if !ok {
...
func (rm *ResourceMap) FindCopy(k ResourceID, val *ResourceStatus) bool {
	rm.rwMu.RLock()
	defer rm.rwMu.RUnlock()
	v, ok := rm.m[k]
	if ok && (v != nil) {
